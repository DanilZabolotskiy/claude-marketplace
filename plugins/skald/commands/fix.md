...
- If either is missing or invalid, do NOT launch the subagent. Print a one-line error showing usage: `/skald:fix <task-id> <pr-number>` and stop.

If both arguments are valid:

1. Resolve the PR's head branch:
   - `gh pr view $2 --json headRefName -q .headRefName` → capture as `<branch>`.
2. Fetch the latest skald review report from PR comments:
   - `gh pr view $2 --json comments --jq '[.comments[] | select(.body | startswith("<!-- skald-pr-reviewer-report"))] | last | .body'`
   - The marker `<!-- skald-pr-reviewer-report -->` is injected by `pr-reviewer` at the top of every report it posts, so this filter only picks skald-authored comments (ignoring unrelated human comments).
   - If the result is empty or null, stop with a one-line error: «На PR #$2 не найдено отчёта skald. Сначала запустите `/skald:review $1 $2`.»
