...
2. Run the full test suite, not only the new tests. Default for Python projects: `pytest`.
...
If a `VERSION` file exists at the project root, increment it as part of the iteration: use the version named in the iteration's Definition of done when one is given, otherwise bump the patch component. If the file does not exist and the iteration does not ask to create it, skip silently.
